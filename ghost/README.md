...
## Older iPod troubleshooting (player v2)

Safari's built-in player appears above the test buttons and starts with Crazy Laugh 1 selected. If a custom button does not start audio, tap the built-in Play control. The direct sound link opens the selected MP3 outside the page's custom playback code. Report the iOS version, the displayed message/error code, and whether either fallback plays. Error code 2 indicates a network failure, 3 a decoding failure, and 4 an unavailable or unsupported source. These fallbacks help diagnose the device; compatibility on every old iOS release is not yet verified.
